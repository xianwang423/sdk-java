...
public class ArbSubscriptionTest extends ApiCoreTestBase {
	...
	public void testGetSubscriptionList() throws DatatypeConfigurationException {
		...
		try {
			ARBSubscriptionStatusEnum newStatus = getSubscription(
					merchantAuthenticationType, subscriptionId);
			Assert.assertEquals(ARBSubscriptionStatusEnum.ACTIVE, newStatus);

			LogHelper.info(logger,
					"Getting Subscription List for SubscriptionId: %s",
					subscriptionId);
			...
			do {
				...
				ARBGetSubscriptionListResponse listResponse = executeTestRequestWithSuccess(
						listRequest, ARBGetSubscriptionListController.class,
						environment);

				LogHelper.info(logger, "Subscription Count: %d",
						listResponse.getTotalNumInResultSet());
				Assert.assertTrue(0 < listResponse.getTotalNumInResultSet());

				ArrayOfSubscription subscriptionsArray = listResponse
						.getSubscriptionDetails();
				Assert.assertNotNull(subscriptionsArray);

				int subsId = Integer.parseInt(subscriptionId);

				for (SubscriptionDetail aSubscription : subscriptionsArray
						.getSubscriptionDetail()) {
					Assert.assertTrue(0 < aSubscription.getId());
					LogHelper
							.info(logger,
									"Subscription Id: %s, Status:%s, PaymentMethod: %s, Amount: %s, Account:%s",
									aSubscription.getId(),
									aSubscription.getStatus(),
									aSubscription.getPaymentMethod(),
									aSubscription.getAmount(),
									aSubscription.getAccountNumber());
					if (subsId == aSubscription.getId()) {
						found = true;
					}
				}
			...
			// validate the status of subscription to make sure it is
			// in-activated
			ARBSubscriptionStatusEnum cancelStatus = getSubscription(
					merchantAuthenticationType, subscriptionId);
			Assert.assertEquals(ARBSubscriptionStatusEnum.CANCELED,
					cancelStatus);
		}
	}
	...
	private ARBGetSubscriptionListRequest setupSubscriptionListRequest(
		...
		listRequest
				.setSearchType(ARBGetSubscriptionListSearchTypeEnum.SUBSCRIPTION_ACTIVE);
	...
	private ARBSubscriptionStatusEnum getSubscription(
			MerchantAuthenticationType merchantAuthentication,
			String subscriptionId) {
		// get a subscription
		...
		ARBGetSubscriptionStatusResponse getResponse = executeTestRequestWithSuccess(
				getRequest, ARBGetSubscriptionStatusController.class,
				environment);
		Assert.assertNotNull(getResponse.getStatus());
		logger.info(String.format("Subscription Status: %s",
	...
	private String createSubscription(
		...
		ARBCreateSubscriptionResponse createResponse = executeTestRequestWithSuccess(
				createRequest, ARBCreateSubscriptionController.class,
				environment);
		Assert.assertNotNull(createResponse.getSubscriptionId());
		logger.info(String.format("Created Subscription: %s",
