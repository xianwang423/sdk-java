...
public abstract class ApiCoreTestBase {
	...
	static {
		...
		// environment
		apiLoginIdKey = UnitTestData.getPropertyFromNames(
				Constants.ENV_API_LOGINID, Constants.PROP_API_LOGINID);
		transactionKey = UnitTestData.getPropertyFromNames(
				Constants.ENV_TRANSACTION_KEY, Constants.PROP_TRANSACTION_KEY);
		md5HashKey = UnitTestData.getPropertyFromNames(
				Constants.ENV_MD5_HASHKEY, Constants.PROP_MD5_HASHKEY);
		...
		if (null != apiLoginIdKey && null != transactionKey) {
			merchant = Merchant.createMerchant(environment, apiLoginIdKey,
					transactionKey);
		}
		if (null == merchant) {
	...
	public void setUp() throws Exception {
		...
		nowString = DateUtil
				.getFormattedDate(now, ReportingDetails.DATE_FORMAT);
		...
		customerProfileType
				.setDescription(getRandomString("CustomerDescription"));
		customerProfileType.setEmail(counterStr
				+ ".customerProfileType@test.anet.net");

		// make sure these elements are initialized by calling get as it uses
		// lazy initialization
		...
		paymentScheduleTypeOne.setStartDate(datatypeFactory
				.newXMLGregorianCalendar(nowDate));
	...
	public static BigDecimal setValidTransactionAmount(long number) {
	...
	private static BigDecimal setValidAmount(long number, int maxAmount) {
		return new BigDecimal(number > maxAmount ? (number % maxAmount)
				: number);
	}
	...
	private static int MAX_TRANSACTION_AMOUNT = 10000;// 214747;
	...
	protected static <Q extends ANetApiRequest, S extends ANetApiResponse, T extends ApiOperationBase<Q, S>> S executeTestRequestWithSuccess(
		...
		S response = executeTestRequest(true, request, controllerClass,
				execEnvironment);
	...
	protected static <Q extends ANetApiRequest, S extends ANetApiResponse, T extends ApiOperationBase<Q, S>> S executeTestRequestWithFailure(
			Q request, Class<T> controllerClass, Environment execEnvironment) {
		S response = executeTestRequest(false, request, controllerClass,
				execEnvironment);
	...
	private static <Q extends ANetApiRequest, S extends ANetApiResponse, T extends ApiOperationBase<Q, S>> S executeTestRequest(
			boolean successExpected, Q request, Class<T> controllerClass,
			Environment execEnvironment) {
		LogHelper.debug(logger, "Created %s Request: '%s'", request.getClass(),
				request);
		...
		try {
			...
			Constructor<T> constructor = controllerClass
					.getConstructor(parameterTypes);
			Object controllerObject = constructor.newInstance(request);
			controller = (T) controllerObject;
			ANetApiResponse baseResponse = controller
					.executeWithApiResponse(execEnvironment);
			LogHelper.info(logger, "%s ResultCode: %s", controllerClass,
					controller.getResultCode());
			LogHelper.info(logger, "%s Results:    %s", controllerClass,
					controller.getResults());
			response = (S) baseResponse;

		} catch (Exception e) {
			LogHelper.error(logger, "Exception : '%s' during %s",
					e.getMessage(), controllerClass);
		}
		if (successExpected) {
		...
		if (null == response && null != controller
				&& null != controller.getErrorResponse()) {
	...
	private static void displayResponse(ANetApiResponse response, String source) {
		...
		if (null != response) {
			...
			if (null != messageType) {
				LogHelper.info(logger, "MessageCode: '%s' ", messageType
						.getResultCode().toString());
				for (Message aMessage : messageType.getMessage()) {
					LogHelper.info(logger, "Message: '%s':'%s' ",
							aMessage.getCode(), aMessage.getText());
				}
			}
		}
	}
	...
	protected static MessagesType.Message getFirstErrorMessage(
	...
	protected <Q extends ANetApiRequest, S extends ANetApiResponse> void setMockControllerExpectations(
			final IApiOperation<Q, S> mockController, final Q mockRequest,
			final S mockResponse, final ANetApiResponse errorResponse,
			final List<String> results, final MessageTypeEnum messageType) {
	...
	public static void showProperties(Object bean) {
		...
		try {
			...
			logger.error(String
					.format("Exception during navigating properties: Message: %s, StackTrace: %s",
							e.getMessage(), e.getStackTrace()));
		}
	}

	public static void processCollections(Class<?> type, String name,
			Object value) {
		if (null != type) {
			if (Collection.class.isAssignableFrom(type)) {
				logger.info(String
						.format("Iterating on Collection: '%s'", name));
